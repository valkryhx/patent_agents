...
import logging
# Try optional Google Generative AI import; fall back to dummy if unavailable
try:
	import google.generativeai as genai
	from google.generativeai.types import HarmCategory, HarmBlockThreshold
	HAS_GOOGLE_GENAI = True
except Exception:  # ImportError or any runtime env errors
	genai = None
	HarmCategory = None
...
class GoogleA2AClient:
	...
	def __init__(self, api_key: Optional[str] = None):
		if not HAS_GOOGLE_GENAI:
			raise ValueError("google.generativeai not available; use DummyA2AClient")
...
# Dummy fallback client (no external dependencies)
class DummyA2AClient:
	"""Fallback AI client that returns deterministic, template-based outputs"""
	def __init__(self):
		logger.warning("Using DummyA2AClient fallback (no external AI calls)")
	
	async def analyze_patent_topic(self, topic: str, description: str) -> PatentAnalysis:
		return PatentAnalysis(
			novelty_score=8.2,
			inventive_step_score=7.9,
			industrial_applicability=True,
			prior_art_analysis=[{"source": "MockDB", "summary": "No identical prior art found; related KG/RAG works present."}],
			claim_analysis={"independent_count": 1, "dependent_count": 4},
			technical_merit={"evidence_graph": "improves answer verifiability and retrieval precision"},
			commercial_potential="High in enterprise QA/compliance search",
			patentability_assessment="Promising",
			recommendations=["Emphasize graph construction pipeline", "Quantify retrieval gains via precision@k"]
		)
	
	async def search_prior_art(self, topic: str, keywords: List[str], max_results: int = 20) -> List[SearchResult]:
		return []
	
	async def generate_patent_draft(self, topic: str, description: str, analysis: PatentAnalysis) -> PatentDraft:
		abstract = (
			f"A Retrieval-Augmented Generation (RAG) system enhanced by an evidence graph that structurally represents sources, "
			f"claims, and support relations, enabling provenance tracking, contradiction detection, and answer verification."
		)
		background = (
			"Technical Field: information retrieval and natural language processing. "
			"Nearest prior art: standard RAG systems that retrieve top-k passages for generation without structured cross-document evidential linking. "
			"Drawbacks: weak provenance, hallucination risk, and limited multi-hop reasoning."
		)
		summary = (
			"The invention introduces an evidence graph constructed from retrieved documents where nodes encode atomic evidence units and edges encode support/contradict/refine relations. "
			"A graph-aware retriever and verifier jointly rank subgraphs to condition the generator and produce verifiable answers with citations."
		)
		desc = (
			"Implementation includes: (1) retrieval and evidence unitization (sentence/claim extraction), "
			"(2) graph construction with relation classification (support/attack/refine), (3) graph pruning and subgraph selection, "
			"(4) graph-conditioned decoding with constrained citation, and (5) post-hoc verification and feedback to the retriever."
		)
		claims = [
			"1. A RAG method characterized by constructing an evidence graph over retrieved documents and conditioning generation on a subgraph selected by a verifier.",
			"2. The method of claim 1, wherein nodes represent atomic evidence units with source provenance and confidence scores.",
			"3. The method of claim 1, wherein edges represent support, contradiction, or refinement relations inferred by a classifier.",
			"4. The method of claim 1, further comprising graph-conditioned decoding that enforces citation alignment to selected nodes.",
			"5. The method of claim 1, wherein a feedback loop updates retrieval ranking based on verification outcomes."
		]
		drawings = [
			"Figure 1: System architecture of evidence-graph-enhanced RAG",
			"Figure 2: Evidence extraction and graph construction pipeline",
			"Figure 3: Subgraph selection and verification flow",
			"Figure 4: Graph-conditioned decoding with citation control",
			"Figure 5: Feedback loop to retriever"
		]
		return PatentDraft(
			title=f"Evidence-Graph Enhanced Retrieval-Augmented Generation System",
			abstract=abstract,
			background=background,
			summary=summary,
			detailed_description=desc,
			claims=claims,
			drawings_description="Brief descriptions of figures 1-5",
			technical_diagrams=drawings,
		)
	
	async def generate_technical_diagrams(self, description: str) -> List[str]:
		return [
			"Figure 1: Overall architecture with retriever, graph builder, verifier, generator",
			"Figure 2: Evidence extraction and relation classification",
			"Figure 3: Subgraph selection and verification"
		]
	
	# The following parse methods are no-ops in dummy; kept for API symmetry if called
	async def _generate_response(self, prompt: str) -> str:
		return "DUMMY"
...
async def get_google_a2a_client() -> Union[GoogleA2AClient, DummyA2AClient]:
	"""Get or create the global A2A client; enforce GLM-4.5-flash only (Google disabled)."""
	...
	if google_a2a_client is None:
		...
		if not glm_key:
			raise ValueError("ZHIPUAI_API_KEY or GLM_API_KEY is required; Google models are disabled")
