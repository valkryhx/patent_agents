...
async def generate_patent_conservative():
    """保守地生成专利内容"""
    
    try:
        print("🚀 开始保守生成专利：基于语义理解的复杂函数参数智能推断与分层调用重试优化方法")
...
## 生成策略
采用保守生成策略，每个章节之间等待足够时间（120-180秒），确保GLM API完全恢复，避免并发限制问题。
"""
...
if __name__ == "__main__":
    print("🔍 开始保守生成专利内容...")
    success = asyncio.run(generate_patent_conservative())
    if success:
        print("✅ 专利生成成功！")
        sys.exit(0)
    else:
