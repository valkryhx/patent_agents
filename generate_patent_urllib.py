...
async def generate_patent_with_urllib():
    """使用urllib方式生成专利内容"""
    
    try:
        print("🚀 开始生成专利：基于语义理解的复杂函数参数智能推断与分层调用重试优化方法")
        
        # 直接使用GLM Client的urllib方式
        from patent_agent_demo.glm_client import GLMA2AClient
        
        client = GLMA2AClient()
        
        # 创建输出目录
        output_dir = "./output/progress/基于语义理解的复杂函数参数智能推断与分层调用重试优化方法_urllib"
...
请为以下专利主题生成专业的专利标题和摘要：

专利主题：基于语义理解的复杂函数参数智能推断与分层调用重试优化方法

要求：
1. 生成一个专业、准确的专利标题
2. 生成中英文对照的专利摘要（中文≤150字，英文≤150字）
3. 突出核心技术特点：语义理解、参数智能推断、分层调用、重试优化
4. 说明技术优势和应用前景
...
"""
        ...
        title_abstract = await client._generate_response(title_abstract_prompt)
        
        # 保存标题和摘要
        with open(f"{output_dir}/00_title_abstract.md", "w", encoding="utf-8") as f:
            f.write("# 标题与摘要\n\n")
...
if __name__ == "__main__":
    print("🔍 开始使用urllib方式生成专利内容...")
    success = asyncio.run(generate_patent_with_urllib())
    if success:
        print("✅ 专利生成成功！")
        sys.exit(0)
    else:
